...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJAccountLoginDelegate> : ModifyBase<ModifyDerive<Der, GJAccountLoginDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int, int>::func(&GJAccountLoginDelegate::loginAccountFinished)), Default, GJAccountLoginDelegate, loginAccountFinished, int, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<AccountError>::func(&GJAccountLoginDelegate::loginAccountFailed)), Default, GJAccountLoginDelegate, loginAccountFailed, AccountError)
		}
	};
}
