...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJMPDelegate> : ModifyBase<ModifyDerive<Der, GJMPDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&GJMPDelegate::joinLobbyFinished)), Default, GJMPDelegate, joinLobbyFinished, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int, GJMPErrorCode>::func(&GJMPDelegate::joinLobbyFailed)), Default, GJMPDelegate, joinLobbyFailed, int, GJMPErrorCode)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&GJMPDelegate::didUploadMPComment)), Default, GJMPDelegate, didUploadMPComment, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&GJMPDelegate::updateComments)), Default, GJMPDelegate, updateComments, )
		}
	};
}
