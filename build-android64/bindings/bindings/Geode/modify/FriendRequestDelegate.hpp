...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, FriendRequestDelegate> : ModifyBase<ModifyDerive<Der, FriendRequestDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<cocos2d::CCArray*, char const*>::func(&FriendRequestDelegate::loadFRequestsFinished)), Default, FriendRequestDelegate, loadFRequestsFinished, cocos2d::CCArray*, char const*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<char const*, GJErrorCode>::func(&FriendRequestDelegate::loadFRequestsFailed)), Default, FriendRequestDelegate, loadFRequestsFailed, char const*, GJErrorCode)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(reinterpret_cast<uintptr_t>(dlsym(dlopen("libcocos2dcpp.so", RTLD_NOW), "_ZN21FriendRequestDelegate13setupPageInfoESsPKc")), Default, FriendRequestDelegate, setupPageInfo, gd::string, char const*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<bool>::func(&FriendRequestDelegate::forceReloadRequests)), Default, FriendRequestDelegate, forceReloadRequests, bool)
		}
	};
}
