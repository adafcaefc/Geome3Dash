...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, UploadMessageDelegate> : ModifyBase<ModifyDerive<Der, UploadMessageDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&UploadMessageDelegate::uploadMessageFinished)), Default, UploadMessageDelegate, uploadMessageFinished, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&UploadMessageDelegate::uploadMessageFailed)), Default, UploadMessageDelegate, uploadMessageFailed, int)
		}
	};
}
