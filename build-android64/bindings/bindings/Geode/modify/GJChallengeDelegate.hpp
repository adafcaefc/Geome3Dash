...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJChallengeDelegate> : ModifyBase<ModifyDerive<Der, GJChallengeDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&GJChallengeDelegate::challengeStatusFinished)), Default, GJChallengeDelegate, challengeStatusFinished, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&GJChallengeDelegate::challengeStatusFailed)), Default, GJChallengeDelegate, challengeStatusFailed, )
		}
	};
}
