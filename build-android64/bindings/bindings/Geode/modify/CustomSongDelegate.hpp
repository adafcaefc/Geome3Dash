...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, CustomSongDelegate> : ModifyBase<ModifyDerive<Der, CustomSongDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&CustomSongDelegate::songIDChanged)), Default, CustomSongDelegate, songIDChanged, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&CustomSongDelegate::getActiveSongID)), Default, CustomSongDelegate, getActiveSongID, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&CustomSongDelegate::getSongFileName)), Default, CustomSongDelegate, getSongFileName, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&CustomSongDelegate::getLevelSettings)), Default, CustomSongDelegate, getLevelSettings, )
		}
	};
}
