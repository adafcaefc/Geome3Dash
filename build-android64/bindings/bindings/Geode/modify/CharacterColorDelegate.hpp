...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, CharacterColorDelegate> : ModifyBase<ModifyDerive<Der, CharacterColorDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<>::func(&CharacterColorDelegate::playerColorChanged)), Default, CharacterColorDelegate, playerColorChanged, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int, UnlockType>::func(&CharacterColorDelegate::showUnlockPopup)), Default, CharacterColorDelegate, showUnlockPopup, int, UnlockType)
		}
	};
}
