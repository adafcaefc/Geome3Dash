...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, DownloadMessageDelegate> : ModifyBase<ModifyDerive<Der, DownloadMessageDelegate>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<GJUserMessage*>::func(&DownloadMessageDelegate::downloadMessageFinished)), Default, DownloadMessageDelegate, downloadMessageFinished, GJUserMessage*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(addresser::getVirtual(Resolve<int>::func(&DownloadMessageDelegate::downloadMessageFailed)), Default, DownloadMessageDelegate, downloadMessageFailed, int)
		}
	};
}
