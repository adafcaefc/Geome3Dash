...
namespace g3d
{
	void Spline::updateParameterList(int points) {
		...
		float maxT = segments.size() * 1.0f - 0.000001f;
	...
	Spline::ParameterData Spline::findClosestByLength(float l) {
		auto it = std::lower_bound(parameterList.begin(), parameterList.end(), l, [](const ParameterData& cp, float l) {
			return cp.l < l;
			});
	...
	glm::vec3 Spline::get(float t) {
		...
		t = std::clamp(t, 0.f, static_cast<float>(segments.size()) - 0.000001f);
